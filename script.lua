...
local function verifyWhitelist()
    ...
    if success then
        ...
        -- On vérifie chaque nom dans la liste
        ...
        -- Si on arrive ici, le nom n'a pas été trouvé
        warn("⚠️ Pseudo '" .. LocalPlayer.Name .. "' non trouvé dans la liste GitHub.")
        print("Contenu actuel du fichier GitHub lu par le script : " .. response)
    else
        warn("❌ Erreur de connexion au serveur de Whitelist : " .. tostring(response))
    end
...
if not verifyWhitelist() then
    -- On affiche un message à l'écran pour être sûr
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "ACCÈS REFUSÉ",
            Text = "Tu n'es pas dans la Whitelist GitHub.",
            Duration = 5
        })
    end)
    return -- Arrête le script de 1000 lignes
end

print("✅ Accès accordé ! Lancement de ton script...")
